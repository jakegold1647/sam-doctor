...
4. Keep the site in step. Error-reference pages are hand-written; a release
   ...
   documented" impression the site gives. `scripts/check-error-pages.py`
   gates this in CI: it fails on a page mapped to a rule id that no longer
   exists, a mapped page that no longer exists, or a page that exists
   without a mapping entry. It does not yet require every rule to have a
   page - `ERROR_PAGE_MAP` covers the 20 rules that already do, and
